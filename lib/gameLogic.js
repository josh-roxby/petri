...
/** withCooldown with a skill-based multiplier applied to the duration (mult < 1 = shorter). */
export function withCooldownScaled(node, action, mult = 1, now = Date.now()) {
  const base = cooldownFor(action, node);
  return {
    ...node,
    cooldowns: { ...(node.cooldowns ?? {}), [action]: now + Math.round(base * Math.max(0.1, mult)) },
  };
}
...
/**
 * Two-parent cross-pollination (tap-tap combine). 50/50 blend with higher
 * noise than single-parent mutation — combinations are chaotic per spec §2.
 * Affinity is inherited from the dominant parent (higher potency wins).
 */
export function mutateFromTwoParents(a, b, rng = Math.random) {
  // Higher chaos+volatility sum = more noise. Capped at ~1.0 for stability.
  const avgVol = (a.volatility + b.volatility) / 2;
  const avgChaos = (a.chaos + b.chaos) / 2;
  const noiseAmp = Math.min(1, (avgVol * 0.5 + avgChaos * 0.4) / 100);
  const noise = () => (rng() - 0.5) * 2 * noiseAmp * 50; // ±0..~25 points
  const blend = (av, bv) => clamp(av * 0.5 + bv * 0.5 + noise(), 0, 100);

  const dominant = (a.potency ?? 0) >= (b.potency ?? 0) ? a : b;

  return {
    potency: Math.round(blend(a.potency, b.potency)),
    purity: Math.round(blend(a.purity, b.purity)),
    toxicity: Math.round(blend(a.toxicity, b.toxicity)),
    volatility: Math.round(
      clamp(randBetween(BIRTH_VOLATILITY.min, BIRTH_VOLATILITY.max, rng) + 5, 0, 100)
    ),
    chaos: Math.round(clamp(avgChaos + (rng() - 0.5) * 30, 0, 100)),
    aff: dominant.aff,
  };
}
 ...
 */
export function rollCatalyseChildren(parent, extraBonus = 0, rng = Math.random) {
  const noP = CHAOS_NO_CHILD_PROB(parent.chaos ?? 0);
  const extraP = Math.min(0.9 - noP, CHAOS_EXTRA_CHILD_PROB(parent.chaos ?? 0) + extraBonus);
  const r = rng();
  if (r < noP) return 0;
 ...
 */
export function applyDiscardCollateral(nodes, targetNodeId, collateralMult = 1, rng = Math.random) {
  const target = nodes.find((n) => n.id === targetNodeId);
  if (!target) return { nodes, damaged: [] };

  const damaged = [];
  const chaosProb = DISCARD_COLLATERAL_PROB(target.chaos ?? 0) * Math.max(0, collateralMult);
...
/**
 * Build a combined child from two parents. `parents: [a.id, b.id]` is set
 * so PetriDish renders edges from both. `parent` stays the dominant one
 * for downstream code that expects a single lineage reference (collapse
 * blast, journal naming, etc.).
 */
export function buildCombinedChildNode({ a, b, id, position, nodes, rng = Math.random }) {
  const inherited = mutateFromTwoParents(a, b, rng);
  // Size is average of parents, slightly smaller.
  const avgR = (a.r + b.r) / 2;
  const r = clamp(Math.round(avgR - 1 + (rng() - 0.5) * 3), 7, 18);
  const dominant = (a.potency ?? 0) >= (b.potency ?? 0) ? a : b;
  return {
    id,
    x: position.x,
